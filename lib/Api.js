...
let dynamic_global_params_type = `2.${parseInt(
  ChainTypes.impl_object_type.dynamic_global_property,
  10
)}.0`
...
class Api {
  ...
  async init () {
    ...
    Apis.setRpcConnectionStatusCallback(async msg => {
      ...
      if (msg === 'closed') {
        Apis.reset(config.apiServer, true)
      }
    })
  ...
  getAccount (uid) {
    return new Promise((resolve, reject) => {
      ...
      ChainStore.fetchAccountByUid(uid)
        .then(uObj => {
          if (null == uObj) {
            return reject({
              code: 2001,
            ...
            .then(res => {
              ...
              uObj.statistics = {
                ...
                total_committee_member_pledge:
                  statistics.total_committee_member_pledge,
                total_platform_pledge: statistics.total_platform_pledge,
                releasing_witness_pledge: statistics.releasing_witness_pledge,
                releasing_committee_member_pledge:
                  statistics.releasing_committee_member_pledge,
   ...
   * @returns {Promise<*>}
   */
  async getLastLicenseId (account) {
    let stat = await ChainStore.fetchAccountStatisticsByUid(account)
    return stat.last_license_sequence
  }
   ...
   * @returns {Promise<*>}
   */
  async getLastAid (platform) {
    let stat = await ChainStore.fetchAccountStatisticsByUid(platform)
    return stat.last_advertising_sequence
  }
  ...
  transfer (
    ...
    let fetchFromKey = new Promise((resolve, reject) => {
      return this.getAccount(from_uid)
        .then(uObj => {
          resolve(uObj)
        })
        .catch(e => {
    ...
    return new Promise((resolve, reject) => {
      if (!utils.isNumber(amount)) {
        ...
        Promise.all([fetchFromKey, fetchToKey, fetchAsset])
          .then(res => {
            ...
            if (memo && memo.trim() != '') {
              ...
              let nonce = long
                .shiftLeft(8)
                .or(Long.fromNumber(entropy))
                .toString()

              // TODO: 用户之间通过平台转账操作，不做签名，因为平台无法获取到用户私钥
              let message =
                config.platform_id == from_uid
            ...
            return tr
              .set_required_fees(from_uid, false, use_csaf)
              .then(() => {
   ...
   * 领积分
   * - 领取某个账户的积分
   * @param {Number} from 积分领取人id
   * @param {Number} to 积分接收人 id
   * @param {Number} amount 积分数量 （不含精度）
   * @returns {Promise<U>|Promise.<T>|*|Promise} resolve(block_num 操作所属块号, txid 操作id), reject(e 异常信息)
   */
  async collectCsaf ({from, to, amount}) {
    ...
    await tr.set_required_fees(from, config.use_csaf, true)
   ...
   * 锁仓
   * - 更新锁仓的抵押
   * @param {Number} uid 锁仓账号
   * @param {Number} amount 锁仓金额
   * @returns {Promise<U>|Promise.<T>|*|Promise} resolve(block_num 操作所属块号, txid 操作id), reject(e 异常信息)
   */
  async lockBalance ({uid, amount }) {

    // todo uid 需要默认是配置里的平台账号

    let op_data = {
      account: uid,
      new_lock_balance: amount
    }
    ...
    let tr = new TransactionBuilder()
    tr.add_type_operation('balance_lock_update', op_data)

    await tr.set_required_fees(uid, config.use_csaf, true)
   ...
   * 领积分
   * - 领取某个账户的积分
   * @param {Number} issuer - 受益人账号 
   * @param {Number} from - 积分生产人账号id
   * @param {Number} to - 积分接受人账号id
   * @param {Number} amount - 积分数量
   * @returns {Promise<U>|Promise.<T>|*|Promise} resolve(block_num 操作所属块号, txid 操作id), reject(e 异常信息)
   */
  async collectBenefitCsaf ({issuer, from, to, amount}) {
    ...
    let tr = new TransactionBuilder()
    tr.add_type_operation('benefit_collect', op_data)

    await tr.set_required_fees(issuer, config.use_csaf, true)
   ...
   * 领积分
   * - 领取某个账户的积分
   * @param {Number} issuer - 受益人账号id
   * @param {Number} from - 见证人账号 id
   * @param {Number} amount - 领取数量
   * @returns {Promise<U>|Promise.<T>|*|Promise} resolve(block_num 操作所属块号, txid 操作id), reject(e 异常信息)
   */
  async collectBenefitWitnessWages ({issuer, from, amount}) {
  ...
  getHistory (uid, op_type = null, start = 0, limit = 10) {
    return this.getAccount(uid)
      .then(uObj => {
        return ChainStore.fetchRelativeAccountHistory(
          uid,
          op_type,
          0,
          limit,
  ...
  getAuthUsers (platform, lower_bound_account = 0, limit = 1) {
    return new Promise((resolve, reject) => {
      Apis.instance()
        .db_api()
        .exec('list_account_auth_platform_by_platform', [
          platform,
          lower_bound_account,
          limit
        ])
  ...
  confirmBlock (block_num) {
    return Apis.instance()
      .db_api()
      .exec('get_objects', [[dynamic_global_params_type]])
      .then(global_params => {
        let irreversible_block_num =
          global_params[0].last_irreversible_block_num
  ...
  async getPostPermissions (origin_platform, origin_poster, origin_post_pid) {
    let post_info = await this.getPost(
      origin_platform,
      origin_poster,
      origin_post_pid
    )
  ...
  async post ({
    ...
    if (origin_post_pid && origin_platform && origin_poster) {
      ...
      let permission_flags = await this.getPostPermissions(
        origin_platform,
        origin_poster,
        origin_post_pid
      )
      ...
      // 不允许转发
      if (
        extensions.post_type === 2 &&
        (utils.postPermissions.Post_Permission_Forward & permission_flags) === 0
      ) {
        return {
          code: 2000,
          message: '文章不允许转发'
      ...
      if (
        extensions.post_type === 1 &&
        (utils.postPermissions.Post_Permission_Comment & permission_flags) === 0
      ) {
        return {
          code: 2000,
          message: '文章不允许评论'
   ...
   */
  async post_simple ({ platform, poster, title, body, url, license_lid }) {
    let extra_data = {
      url
    }
  ...
  getPost (platform, poster, post_pid) {
    return new Promise((resolve, reject) => {
      Apis.instance()
        .db_api()
        .exec('get_post', [platform, poster, post_pid])
        .then(post => {
          resolve(post)
        })
        .catch(e => {
   ...
   * @param {String} [该参数已废弃]start 开始时间 - 'yyyy-MM-ddThh:mm:ss' ISOString
   * @param lower_bound, 逆序查询。默认1.8.0 即逆序查询所有 1.7.x ; 若用户指定 1.7.x , 即查询1.7.x之前的符合条件文章
  ...
  getPostList (
    ...
    return new Promise((resolve, reject) => {
      Apis.instance()
        .db_api()
        .exec('get_posts_by_platform_poster', [
          platform,
          poster,
          lower_bound,
          limit
   ...
   * @returns {Promise<any>}
   */
  getPostProfit ({ begin_period, end_period, platform, poster, pid }) {
    return this.proxy('db_api', 'get_post_profits_detail', [
      begin_period,
      end_period,
      platform,
      poster,
  ...
  getPosterProfit ({
    ...
    return this.proxy('db_api', 'get_poster_profits_detail', [
      begin_period,
      end_period,
      poster,
      lower_bound_index,
  ...
  getPlatformProfit ({
    ...
    return this.proxy('db_api', 'get_platform_profits_detail', [
      begin_period,
      end_period,
      platform,
      lower_bound_index,
   ...
   */
  scoreCreate ({ from_account, platform, poster, pid, score, csaf }) {
    return new Promise((resolve, reject) => {
      let op_data = {
      ...
      tr.set_required_fees(op_data.from_account_uid, false, true)
        .then(() => {
          tr.add_signer(PrivateKey.fromWif(config.secondary_key))
          this.__broadCast(tr)
            .then(res => resolve(res))
   ...
   */
  rewardProxy ({ from_account, platform, poster, pid, amount }) {
    return new Promise((resolve, reject) => {
      let op_data = {
        from_account_uid: from_account,
        platform: platform,
   ...
   * @param {Number|String} from_account 打分人
   */
  getPostScore ({ platform, poster, pid, from_account }) {
    return this.proxy('db_api', 'get_score', [
      platform,
      poster,
      pid,
      from_account
  ...
  listPostScores ({
    ...
    return this.proxy('db_api', 'list_scores', [
      platform,
      poster,
      pid,
      lower_bound_score,
  ...
  /**
   * 创建广告位
   * @param {Number} platform 平台账号
   * @param {String} description 广告位描述
   * @param {Number} unit_price 单位时间价格
   * @param {Number} unit_time 单位时间
   * @returns  {Promise<U>|Promise.<T>|*|Promise} resolve(block_num 操作所属块号, txid 操作id), reject(e 异常信息)
   */
  async createAdvertising ({
    ...
    op_data.advertising_aid = (await this.getLastAid(platform)) + 1
    let tr = new TransactionBuilder()
    tr.add_type_operation('advertising_create', op_data)
    await tr.set_required_fees(platform, false, true)
    let tr_obj = tr.toObject()
    let fee = tr_obj.operations[0][1].fee
    op_data.fee = fee
    ...
    common_return.advertising = op_data
    return common_return
  }
  /**
   * 更新广告位
   * @param {Number} platform 平台账号
   * @param {Number} advertising_aid 广告位id
   * @param {String} description 广告位描述
   * @param {Number} unit_price 单位时间价格
   * @param {Number} unit_time 单位时间
   * @param {Boolean} on_sell 出售状态
   * @returns  {Promise<U>|Promise.<T>|*|Promise} resolve(block_num 操作所属块号, txid 操作id), reject(e 异常信息)
   */
  async updateAdvertising ({
    ...
    let tr = new TransactionBuilder()
    tr.add_type_operation('advertising_update', op_data)
    await tr.set_required_fees(platform, false, true)
    let tr_obj = tr.toObject()
    let fee = tr_obj.operations[0][1].fee
    op_data.fee = fee
   ...
   * @param {Number} platform 平台账号
   * @param {Number} advertising_aid 广告位id
   * @param {Number} advertising_order_oid 广告位订单id
   * @param {Boolean} isconfirm 是否确认
   * @returns  {Promise<U>|Promise.<T>|*|Promise} resolve(block_num 操作所属块号, txid 操作id), reject(e 异常信息)
   */
  async confirmAdvertising ({
    platform = config.platform_id,
    advertising_aid,
    advertising_order_oid,
    isconfirm
  }) {
    let op_data = {
      platform,
    ...
    let tr = new TransactionBuilder()
    tr.add_type_operation('advertising_confirm', op_data)
    await tr.set_required_fees(platform, false, true)
    let tr_obj = tr.toObject()
    let fee = tr_obj.operations[0][1].fee
    op_data.fee = fee
   ...
   * @param {Number|String} platform 平台 yoyow 账号
   * @param {Number} limit 查询条数
   * @param lower_bound_advertising 起始广告id
   * @returns {Promise<U>|Promise.<T>|*|Promise} resolve(list 文章列表), reject(e 异常信息)
   */
  getAdvertisingList ({ platform, lower_bound_advertising, limit }) {
    return this.proxy('db_api', 'list_advertisings', [
      platform,
  ...
  /**
   * 获取购买者广告订单列表
   * @param {Number|String} purchaser 广告购买者id
   * @param {Number} limit 查询条数
   * @param lower_bound_advertising_order 起始广告订单id
   * @returns {Promise<U>|Promise.<T>|*|Promise} resolve(list 文章列表), reject(e 异常信息)
   */
  listAdsOrdersByPurchaser ({
    purchaser,
    lower_bound_advertising_order,
    limit
  }) {
    return this.proxy('db_api', 'list_advertising_orders_by_purchaser', [
      purchaser,
      lower_bound_advertising_order,
      limit
    ])
  ...
  /**
   * 获取购买者广告订单列表
   * @param {Number|String} platform 广告所在平台id
   * @param lower_bound_advertising_order 起始广告订单id
   * @param advertising_aid_type {Number} 广告位id
   * @param {Number} limit 查询条数
   * @returns {Promise<U>|Promise.<T>|*|Promise} resolve(list 文章列表), reject(e 异常信息)
   */
  listAdsOrdersByAdsAid ({
    platform,
    advertising_aid_type,
    lower_bound_advertising_order,
    limit
  }) {
    return this.proxy('db_api', 'list_advertising_orders_by_ads_aid', [
      platform,
      advertising_aid_type,
      lower_bound_advertising_order,
      limit
   ...
   * 创建许可
   * @param {Number|String} platform 平台 yoyow 账号
   * @param {uint8} type 许可的类型
   * @param {string} hash_value 许可的hash
   * @param {string} title 许可的标题
   * @param {string} body 许可的内容
   * @param {string} extra_data 许可的额外内容
   */
  async createLicense ({
    platform,
    type,
    ...
    platform = platform || config.platform_id
    let license_lid = (await this.getLastLicenseId(platform)) + 1
    let op_data = {
      platform,
      type,
      hash_value,
    ...
    console.log(op_data)
    let tr = new TransactionBuilder()
    tr.add_type_operation('license_create', op_data)
    await tr.set_required_fees(platform, false, true)
    let tr_obj = tr.toObject()
    let fee = tr_obj.operations[0][1].fee
    op_data.fee = fee
  ...
  __broadCast (tr) {
    return new Promise((resolve, reject) => {
      ...
      return tr
        .broadcast(() => resolve(common_return(tr)))
  ...
  proxy (module, method, params, cb = null) {
    return new Promise((resolve, reject) => {
      ...
      [module]()
        ...
        .then(rsp => {
          resolve(rsp)
          typeof cb === 'function' ? resolve(cb(rsp)) : resolve(rsp)
        })
        .catch(reason => {
          reject(reason)
        })
