...
class Api {
  ...
  async getPostPermissions(origin_platform, origin_poster, origin_post_pid) {
    return self.getPost(origin_platform, origin_poster, origin_post_pid).permission_flags
  }
  ...
  async post({
    ...
    if (origin_post_pid && origin_platform && origin_poster) {
      ...
      permissions_flag = await self.getPostPermissions(origin_platform, origin_poster, origin_post_pid);

      // 不允许转发
      if (!(ext.post_type === 1 && (utils.postPermissions.Post_Permission_Forward & permissions_flag))) {
        reject({
          code: 2000,
          message: "文章不允许转发"
        });
      }
      if (!(ext.post_type === 2 && (utils.postPermissions.Post_Permission_Comment & permissions_flag))) {
        reject({
          code: 2000,
          message: "文章不允许评论"
        });
