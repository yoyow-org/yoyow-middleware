...
class Api {
    ...
    getAccount(uid) {
        return new Promise((resolve, reject) => {
            if (! AccountUtils.validAccountUID(uid)) {
                return reject({code: 2002, message: '无效的账号'});
            }
            ChainStore.fetchAccountByUid(uid).then(uObj => {
                if (null == uObj) {
                   return reject({code: 2001, message: '账号不存在'});
                }
   ...
   */
     ...
     * @returns {Promise<U>|Promise.<T>|*|Promise} resolve(block_num 操作所属块号, txid 操作id), reject(e 异常信息)
     */
    post(platform, poster, post_pid, title, body, extra_data, hash_value=null, origin_platform = null, origin_poster = null, origin_post_pid = null){
        return new Promise((resolve, reject) => {
            let op_data = {
                post_pid: post_pid,
                platform: platform,
     ...
     */
    async post_simple({platform, poster, title, body, url, license_lid, origin_platform, origin_poster, origin_post_pid}) {
      let extra_data = { url };
    ...
    async postUpdate__simple({platform, poster, title, post_pid, body, url, license_lid}) {
      let extra_data = { url };
      ...
      try {
        ...
        return {code: 2000, message: e.message}
      }
    }
    ...
    async getInfo() {
      return await this.proxy('db_api', 'get_objects', [[dynamic_global_params_type]]);
    }
    ...
    */
    listPostScores({platform, poster, pid, limit, list_cur_period, lower_bound_score}) {
